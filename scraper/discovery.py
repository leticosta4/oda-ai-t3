...
from scraper.utils import random_sleep, sleep, clean_str
...
async def handle_not_disabled(page: Page, btn: Locator):
    ...
    first_item_before = await first_item.text_content() if await first_item.count() > 0 else ""
    await btn.click()
...
async def prepare_search_page(page: Page, chave: str):
    ...
    await page.locator("ul.ui-selectonemenu-items li").get_by_text("Nordeste", exact=True).click()
    await page.wait_for_load_state('networkidle')

    # Seleciona UF: Bahia
    await page.locator("div[id='idFormConsultaParametrizada:idUF']").click()
    await page.locator("ul.ui-selectonemenu-items li").get_by_text("Bahia", exact=True).click()
    await page.wait_for_load_state('networkidle')
        ...
        for key in keys:
            try:
                ...
                while True:
                    ...
                    botoes_espelho = await page.locator("a[id*='idBtnVisualizarEspelhoGrupo']").all()
                        ...
                        try:
                            ...
                            for lbl in labels:
                                ...
                                if "Instituição" in lbl_txt:
                                    val_el = item_li.locator(f"label:has-text('{lbl_txt}') + div")
                                    if await val_el.count() > 0:
                                        instituicao = clean_str(await val_el.first.inner_text())
                                elif "Área" in lbl_txt:
                                    val_el = item_li.locator(f"label:has-text('{lbl_txt}') + div")
                                    if await val_el.count() > 0:
                                        area = clean_str(await val_el.first.inner_text())
                            ...
                            await aba.wait_for_selector("#tituloImpressao", timeout=10000)
                            
                            conteudo_html = await aba.content()
                            match = re.search(r'espelhogrupo/(\d{16})', conteudo_html)
                            if match:
                                dgp_id = match.group(1)
                                await group_queue_discovery(dgp_id, nome_grupo, area, instituicao)
                                print(f"   [+] Grupo cadastrado: {nome_grupo} ({dgp_id})")

                        except Exception as e:
                            print(f"    [-] Erro ao processar item ({index}/{total_nesta_pagina}): {str(e)}")
                        finally:
                    ...
                    is_disabled = "ui-state-disabled" in (await proximo_btn.first.get_attribute("class") or "")
                    if is_disabled:
