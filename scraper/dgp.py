...
from scraper.database import (
    update_group_queue_status, update_pesquisador_queue_status
)
...
async def scrape_group_page(context: BrowserContext, page: Page, dgp_id: str):
    ...
    url_alvo = f"http://dgp.cnpq.br/dgp/espelhogrupo/{dgp_id}"
    ...
    html_popups = []
    botoes_linha = await page.locator("a[id*='idBtnVisualizarEspelhoLinhaPesquisa']").all()
    ...
    data = extractor.extrair_html_espelho(html_espelho, html_popups, membro_detalhes_map)
    data["id_dgp"] = dgp_id
...
async def run_dgp_scraper(dgp_ids: list = []):
    ...
    print(f"[Scraper] Encontrados {len(pending_groups)} grupos pendentes. Iniciando extração...")
    ...
    async with async_playwright() as p:
        ...
        for group in pending_groups:
            ...
            try:
                ...
                print(f"[Scraper] Erro crítico no handler para o grupo '{dgp_id}': {str(e)}")
                await update_group_queue_status(dgp_id, "CONCLUIDO")
    ...
    if pending_researchers:
        print(f"[Scraper] Iniciando extração Lattes sequencial para {len(pending_researchers)} pesquisadores pendentes...")
